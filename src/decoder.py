...
class AttentionHead(nn.Module):
    ...
    def scaled_dot_product_attention(self, q, k, v, mask=None):
        ...
        dim_k = k.size(-1)
        scores = q @ k.transpose(1,2) / torch.sqrt(torch.tensor(dim_k, dtype=q.dtype)) # (batch_size, seq_len, seq_len)
        ...
        output = weights @ v   # (batch, seq_len, dim_v)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int, num_attention_heads: int):
        ...
        self.dk = d_model // num_attention_heads
        self.d_model = d_model
        self.heads = nn.ModuleList([AttentionHead(d_model, self.dk, self.dk, self.dk) for i in range(num_attention_heads)])
    ...
    def forward(self, hidden_state, mask=None):
        ...
        if self.d_model % len(self.heads) != 0:
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        att_output = self.self_attention(self.layer_norm_1(x), mask)
        res_1 = att_output + x
        ff_output = self.feed_forward(self.layer_norm_2(res_1))
        x = ff_output + res_1
        return x
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        mask = torch.tril(torch.ones(seq_len, seq_len, device=input_ids.device)).unsqueeze(0).repeat(batch_size, 1, 1)
